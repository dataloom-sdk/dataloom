...
/**
 * Immutable configuration for [InboundPullSynchronizationPipeline].
 *
 * ## Purpose
 *
 * [InboundPullPipelineConfiguration] bounds a single inbound pull pipeline
 * execution. It selects the entity types requested from the remote endpoint
 * and limits both the size of each batch and the number of batches attempted
 * during one [InboundPullSynchronizationPipeline.execute] call.
 *
 * ## Entity types
 *
 ...
 * ## Batch limits
 *
 * [maxEventsPerBatch] bounds the number of events requested from the remote
 * endpoint for a single
 * [io.dataloom.api.transport.PullChangesRequest]. It must be greater than
 * zero.
 *
 * [maxBatchesPerExecution] bounds the number of batches
 * [InboundPullSynchronizationPipeline] attempts during a single `execute`
 ...
 *
 * @param entityTypes optional restriction on the entity types pulled from the
 *   remote endpoint. Defaults to an empty set, meaning all supported entity
 *   types are eligible. The supplied set is defensively copied.
 * @param maxEventsPerBatch maximum number of events requested per pull. Must
 *   be greater than zero. Defaults to `100`.
...
public class InboundPullPipelineConfiguration(
    ...
    init {
        require(maxEventsPerBatch > 0) {
            "InboundPullPipelineConfiguration maxEventsPerBatch must be greater than zero, " +
                "but was $maxEventsPerBatch."
        }
        require(maxBatchesPerExecution > 0) {
            "InboundPullPipelineConfiguration maxBatchesPerExecution must be greater than zero, " +
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other !is InboundPullPipelineConfiguration) return false
