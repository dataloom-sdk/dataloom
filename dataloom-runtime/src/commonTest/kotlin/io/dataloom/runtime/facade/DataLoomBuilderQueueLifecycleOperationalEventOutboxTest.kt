 ...
 * `queueLifecycleOperationalEventOutboxConfiguration` is a real, reachable
 * caller of [io.dataloom.api.operational.DurableOperationalEventOutbox] for a
 * real durable queue-entry transition produced by a real
 * [DataLoomBuilder]-built [io.dataloom.api.facade.DataLoom] instance's queue
 * worker -- and that a durable-recording failure never changes the real
 * [QueueWorkerRunResult] the caller receives, and that nothing is bridged
 * unless both [queueWorkerConfiguration][DataLoomBuilder.queueWorkerConfiguration]
 * and this spec are configured.
 */
class DataLoomBuilderQueueLifecycleOperationalEventOutboxTest {
    ...
    @Test
    fun successfulQueueEntry_appendsRealEntry_whenBothQueueWorkerAndOutboxAreConfigured() = runTest {
        val queue = RecordingQueueProvider()
        val outboxStore = InMemoryOperationalEventOutboxStore()
        val scope = OperationalEventOutboxScope("integration-queue-lifecycle-events")
    ...
    @Test
    fun nothingIsBridged_whenOutboxIsConfiguredButQueueWorkerIsNot() = runTest {
        val queue = RecordingQueueProvider()
        val outboxStore = InMemoryOperationalEventOutboxStore()
        val scope = OperationalEventOutboxScope("integration-no-worker-events")
        val dataLoom = builder(queue)
            .pipeline(SucceedingPipeline(SynchronizationDirection.PUSH))
            // Note: queueWorkerConfiguration is never called, so no queue-entry
            // transition is ever persisted to bridge.
            .queueLifecycleOperationalEventOutboxConfiguration(
                DataLoomQueueLifecycleOperationalEventOutboxSpec(store = outboxStore, scope = scope),
            )
            .build()
    ...
    @Test
    fun nothingIsBridged_whenQueueWorkerIsConfiguredButOutboxIsNot() = runTest {
        val queue = RecordingQueueProvider()
        val outboxStore = InMemoryOperationalEventOutboxStore()
    ...
    @Test
    fun appendFailureDoesNotChangeTheRealQueueProcessingResult() = runTest {
        val queue = RecordingQueueProvider()
    ...
    private fun builder(queue: RecordingQueueProvider): DataLoomBuilder = DataLoomBuilder()
        ...
        .defaultProviderBindings(
            SynchronizationProviderBindings(
                storageProviderId = ProviderId("queue-lifecycle-storage"),
                transportProviderId = ProviderId("queue-lifecycle-transport"),
    ...
    private fun queueWorkerSpec(): DataLoomQueueWorkerSpec = DataLoomQueueWorkerSpec(
        ...
        retryPolicy = object : RetryPolicy {
            override val id = RetryPolicyId("queue-lifecycle-no-retry")
            override fun evaluate(request: RetryEvaluationRequest): RetryDecision =
                RetryDecision.Stop(RetryStopReason.NON_RECOVERABLE)
        },
        retryOperation = RetryOperation("queue-lifecycle.test.operation"),
        configuration = QueueWorkerConfiguration(
            scheduleId = ScheduleId("queue-lifecycle-worker"),
    ...
    private fun runtimeDependencies(): RuntimeDependencies = RuntimeDependencies(
        ...
        identifiers = RuntimeIdentifierGenerators(
            synchronizationEventIds = generator { SynchronizationEventId("queue-lifecycle-event") },
            queueEntryIds = generator { QueueEntryId("queue-lifecycle-generated-entry") },
            queueLeaseIds = generator { QueueLeaseId("queue-lifecycle-generated-lease") },
            conflictIds = generator { ConflictId("queue-lifecycle-conflict") },
        ),
    )
    ...
    private class RecordingQueueProvider : QueueProvider {
        ...
        override val descriptor: ProviderDescriptor = ProviderDescriptor(
            id = ProviderId("queue-lifecycle-queue"),
            name = ProviderName("Queue Lifecycle Test Queue"),
        ...
        override suspend fun acquire(request: QueueAcquireRequest): ProviderOperationResult<QueueAcquireResult> {
            ...
            val entryRequest = SynchronizationRequest(
                workflowId = WorkflowId("queue-lifecycle-workflow"),
                sessionId = SynchronizationSessionId("queue-lifecycle-session"),
                direction = SynchronizationDirection.PUSH,
                mode = SynchronizationMode.DELTA,
                context = ExecutionContext(
                    executionId = ExecutionId("queue-lifecycle-execution"),
                    correlationId = CorrelationId("queue-lifecycle-correlation"),
                ),
            )
            ...
            val entry = QueueEntry(
                id = QueueEntryId("queue-lifecycle-real-entry"),
    ...
    private class RecordingStorageProvider : StorageProvider {
        override val descriptor: ProviderDescriptor = ProviderDescriptor(
            id = ProviderId("queue-lifecycle-storage"),
            name = ProviderName("Queue Lifecycle Storage"),
    ...
    private class RecordingTransportProvider : TransportProvider {
        override val descriptor: ProviderDescriptor = ProviderDescriptor(
            id = ProviderId("queue-lifecycle-transport"),
            name = ProviderName("Queue Lifecycle Transport"),
