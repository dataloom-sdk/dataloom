...
|---:|---|---|---|---|
| 0 | [DL-039 foundations, artifacts, compatibility](https://github.com/dataloom-sdk/dataloom/issues/93) | PARTIAL | Public API/provider/runtime boundaries, module rules, JVM and Kotlin/Native ABI baselines, external-consumer compilation, durable queue/retry/circuit state primitives, canonical operational envelope/redaction and deterministic bounded V1 wire codec/upcast registry, production wall-clock/monotonic-clock implementations for JVM, Android, and iOS (`#221`, 2026-08-08), a `DataLoomSecureRandom` cryptographic randomness boundary for JVM, Android, and iOS (`#230`, 2026-08-09), integrity/key-reference primitives — `DataLoomDigestCalculator`, `DataLoomHmacCalculator`, `KeyReference` — for JVM, Android, and iOS (`#234`, 2026-08-09), a bounded first slice of versioned configuration snapshots/precedence/rollback — `DataLoomConfigurationResolver`, `ConfigurationSnapshot`, `DataLoomConfigurationHistory` (`#236`, 2026-08-09), a bounded first slice of a deterministic policy foundation — `PolicyEvaluator`, `PolicySet`, `PolicyCheckOutcome` (`#239`, 2026-08-09), a security-defaults audit fixing default-`toString()` secret/credential exposure risk across all 32 `DataLoomError` implementations (`#251`, 2026-08-10), a domain-agnostic durable-state contract — `DurableStateStore<TScope, TState>`, generalizing `CircuitBreakerStateStore`'s proven atomic load/compare-and-set pattern with schema-version/migration support (`#253`, 2026-08-10), and its first real domain adoption — `DurableConfigurationHistory` for configuration snapshot history, a checksum-verified reference codec, and a production `RoomDurableStateStore<TScope, TState>` platform implementation reusable by any future domain (`#254`, 2026-08-10) | Adopting the durable-state contract for policy decisions, the only domain still without real persistence (conflict/events/assets/audit remain further out); an Apple file-backed `DurableStateStore` implementation (only Room exists so far); switching real runtime call sites from `DataLoomConfigurationHistory` (in-memory) to the new durable variant; SDK-wide adoption of the secure-random/digest/HMAC/policy primitives at real call sites (including migrating `RetryPolicy`/`StrategyPolicy` onto the new policy foundation); an explicit KMP Android target; the final published artifact graph/BOM (the graph itself is already fully specified in `ADR-0002`; building it as ~20 separate enforced modules is the remaining work); and `message`-content redaction (today only bounded by truncation, not actually sanitized) |
