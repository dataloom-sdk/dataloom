...
| 1 | [DL-039B six strategy engine](https://github.com/dataloom-sdk/dataloom/issues/102) | IN PROGRESS | Versioned contracts and deterministic planner for all six strategies; direct network-only and remote-first slices; a bounded first slice of cache-first — serve-from-cache and synchronous (non-durable) refresh, with the durable/scheduled refresh branch explicitly rejected rather than silently misexecuted (`#249`, 2026-08-10); fail-closed queue admission; Room v8/Apple v4 accepted-plan persistence; exact encoder/resolver correspondence; direct, protected, and queued replay without current-policy evaluation; typed fallback and reconciliation hooks | Wire durable queue admission for cache-first's default (`requireDurableRefresh = true`) scheduled-refresh branch, complete atomic offline-first intent/outbox admission, hybrid coherence/conflict application, durable strategy events/diagnostics, and the full native Android/KMP Android/KMP iOS failure/restart matrix |
