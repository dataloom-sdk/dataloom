...
| 4 | [DL-041 conflict engine](https://github.com/dataloom-sdk/dataloom/issues/95) | IN PROGRESS | Custom detector/resolver contracts and orchestration foundations, plus a bounded first slice of durable unresolved-conflict persistence — `DurableUnresolvedConflictLog` (`#256`, 2026-08-11) — with a real caller, `DurableConflictDetectionCoordinator` (`#257`, 2026-08-11), not yet wired into any `SynchronizationPipeline` | Ship deterministic built-in strategies, decision application, durable resolved-decision persistence, wiring `DurableConflictDetectionCoordinator` into a real synchronization pipeline, audit, loop prevention/quarantine, precedence, restart/concurrency proof, and AC-FUNC-002 |
