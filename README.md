...
| 1 | [DL-039B six strategy engine](https://github.com/dataloom-sdk/dataloom/issues/102) | IN PROGRESS | Versioned contracts and deterministic planner for all six strategies; direct network-only, remote-first, cache-first (bounded first slice — serve-from-cache and synchronous refresh, `#249`, 2026-08-10), offline-first (`#259`, 2026-08-11), hybrid (`#261`, 2026-08-12), and now adaptive (`#263`, 2026-08-12) — all six strategies now execute correctly, five with dedicated executors and adaptive verified end-to-end routing through them with no dedicated executor needed by design; that verification also caught and fixed a real `request.profile` unsafe-cast `ClassCastException` bug in the already-shipped `RemoteFirstStrategyExecutor` (would have hit hybrid too); fail-closed queue admission; Room v8/Apple v4 accepted-plan persistence; exact encoder/resolver correspondence; direct, protected, and queued replay without current-policy evaluation; typed fallback and reconciliation hooks, the latter now genuinely exercised outside the durable-continuation path by offline-first's `RECONCILE` branch | Wire durable queue admission for cache-first's, offline-first's, and hybrid's shared default durable-work branch; add a transport-free result type for hybrid's rejected local-only-PUSH branch; complete hybrid coherence/conflict application, durable strategy events/diagnostics, and the full native Android/KMP Android/KMP iOS failure/restart matrix |
