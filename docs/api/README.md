...
| [Configuration resolver caller investigation](./configuration-resolver-caller-investigation.md) | Investigated, not achievable yet | Why no genuine real caller for `DataLoomConfigurationResolver`/`DurableConfigurationHistory` exists in `dataloom-runtime` today, and what would need to exist first. |
| [Digest/HMAC/secure-random adoption investigation](./digest-hmac-secure-random-adoption-investigation.md) | Investigated, no genuine gap remains | Why `DataLoomDigestCalculator`/`DataLoomHmacCalculator`/`DataLoomSecureRandom` have no further genuine real call site to adopt today — digest adoption is already complete where a real need exists, no message-authentication need exists yet, and the only secure-random consumers are illustrative reference fixtures, with production identifier generation explicitly deferred by design. |
| [Asset manifest](./asset-manifest.md) | Available contract (bounded first slice) | Versioned asset identity/size/media-type/checksum plus chunk geometry and optional compression/encryption labels — `AssetManifest`, `AssetChunkLayout`, `AssetCompressionMetadata`, `AssetEncryptionMetadata`. Upload/download, streaming, resumable sessions, quotas, cancellation, and content-policy hooks remain fully open. |
| [Durable asset manifest history](./durable-asset-manifest-history.md) | Available contract (bounded second slice) | Durable, monotonic, bounded-retention history of an asset's applied `AssetManifest` revisions — `DurableAssetManifestHistory`, the fifth real `DurableStateStore` domain adoption, reusing `RoomDurableStateStore` with zero new Room code. No real caller yet. |
| [`AssetProvider` contract investigation](./asset-provider-contract-investigation.md) | Investigated, not achievable yet | Why no genuinely narrow, decision-free, non-redundant `AssetProvider` contract slice exists yet — `StorageProvider`'s own "narrow interface" depended on already-settled domain vocabulary that assets don't have, the one candidate free of open design questions (`current(assetId)`) is already served by `DurableAssetManifestHistory` with no known second implementation or caller, and issue `#97` itself says V1 is "not a contract-only SPI." |
| [Asset synchronization caller investigation](./asset-synchronization-caller-investigation.md) | Investigated, still blocked | Re-approached `#97` from a real-caller angle instead of a provider-contract angle and converged on the same `FR-ASSET-002`/`-003` blocker. New finding along the way: per-chunk integrity verification (`FR-ASSET-004`) is already fully achievable today from existing `DataLoomDigestCalculator`/`DataLoomDigest` primitives with zero new code, while whole-object verification is blocked by a real, previously unnamed gap — no streaming/incremental digest capability exists, and adding one would reverse an already-documented deliberate one-shot design decision on the Apple target. |
...
| [Conflict resolver policy precedence investigation](./conflict-resolver-policy-precedence-investigation.md) | Investigated, no bounded slice yet | Entity type and workflow ID reliably reach a resolver's `resolve()` call today (tenant ID only when a host supplies it), but none of the three reach `ConflictResolverRegistry`'s exact-ID *selection* step, and that boundary is an explicit, repeatedly-stated invariant across `ConflictResolverRegistry`, `SynchronizationConflictOrchestrator`, and `DataLoomConflictDetectionSpec` — not a missing wire between an existing producer and consumer. Even the narrowest entity-only precedence tier would reverse all three statements with no real caller to validate the new shape against. |
