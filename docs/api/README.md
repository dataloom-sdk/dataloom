...
| [Conflict resolver policy precedence investigation](./conflict-resolver-policy-precedence-investigation.md) | Superseded by design decision D11 (2026-09-19) | Historical record of why exact-ID-only resolver selection needed a design decision. Decision: reversed; an optional `ConflictResolverSelectionPolicy` (entity type > workflow > tenant > global default) now ships — see [Conflict resolution strategies](./conflict-resolution-strategies.md), "Resolver selection policy". |
